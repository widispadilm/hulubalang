...
export function AssignTripForm({ tripId, drivers }: { tripId: string; drivers: InternalUserRef[] }) {
  ...
  return (
    <form action={action} className="mt-2 flex flex-wrap items-end gap-2 rounded-md bg-slate-50 p-3">
      <div>
        <label className="block text-xs font-medium text-slate-600">Driver</label>
        <select name="driverId" required className="mt-1 rounded-md border border-slate-300 px-2 py-1.5 text-sm">
      ...
      <div>
        <label className="block text-xs font-medium text-slate-600">ETA</label>
        <input name="eta" type="datetime-local" required className="mt-1 rounded-md border border-slate-300 px-2 py-1.5 text-sm" />
      </div>
      <div>
        <label className="block text-xs font-medium text-slate-600">Jenis Pengiriman</label>
        <select name="shipmentType" className="mt-1 rounded-md border border-slate-300 px-2 py-1.5 text-sm">
