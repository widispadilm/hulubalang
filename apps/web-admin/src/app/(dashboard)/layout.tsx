...
const NAV = [
  { href: '/dashboard', label: 'Dashboard', icon: LayoutDashboard, roles: ['ADMIN', 'MARKETING', 'OPERATION', 'FINANCE', 'MANAGEMENT', 'DRIVER', 'POOL_KEEPER'] },
  { href: '/orders', label: 'Orders', icon: ShoppingCart, roles: ['ADMIN', 'MARKETING', 'OPERATION', 'FINANCE', 'MANAGEMENT'] },
  { href: '/trips', label: 'Trips', icon: MapIcon, roles: ['ADMIN', 'OPERATION', 'MANAGEMENT', 'FINANCE'] },
  { href: '/checkpoints', label: 'Verifikasi Pool', icon: CheckSquare, roles: ['ADMIN', 'OPERATION', 'POOL_KEEPER'] },
  { href: '/my-trips', label: 'Tugas Saya', icon: Truck, roles: ['DRIVER'] },
  { href: '/pools', label: 'Master Pool', icon: Warehouse, roles: ['ADMIN', 'OPERATION'] },
];
    ...
    <div className="flex min-h-screen bg-pss-base pb-16 md:pb-0">
      ...
      <aside className="hidden md:flex flex-col w-64 shrink-0 border-r border-slate-200 bg-white p-4 shadow-sm z-10">
        ...
        <form action={logoutAction} className="mt-6 border-t border-slate-100 pt-4 px-2">
          <button type="submit" className="group flex w-full items-center gap-3 text-sm font-medium text-slate-500 hover:text-red-600 transition-colors">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around bg-white/80 backdrop-blur-md border-t border-slate-200 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        ...
        <form action={logoutAction}>
          <button type="submit" className="flex flex-col items-center gap-1 p-3 text-slate-500 hover:text-red-600 transition-colors active:scale-95">
