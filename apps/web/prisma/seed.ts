...
async function upsertInternalUser(email: string, name: string, role: InternalRole) {
...
async function main() {
  ...
  const poolBekasi = await prisma.pool.upsert({
    ...
    create: {
      id: 'seed-pool-bekasi',
      name: 'Pool Bekasi (Pusat)',
      address: 'Jl. Raya Kalimalang No.40, Bekasi Timur',
    },
  });
  ...
  // --- User internal ---
  await upsertInternalUser('admin@pss.co.id', 'Admin PSS', 'ADMIN');
  ...
  const keeperBekasi = await upsertInternalUser(
    'keeper.bekasi@pss.co.id',
    'Joko (Penjaga Pool Bekasi)',
    'POOL_KEEPER',
  );
  const keeperSurabaya = await upsertInternalUser(
    'keeper.surabaya@pss.co.id',
    'Wati (Penjaga Pool Surabaya)',
    'POOL_KEEPER',
  );

  // Tugaskan tiap penjaga ke pool-nya masing-masing
  await prisma.pool.update({
    where: { id: poolBekasi.id },
    data: { keepers: { connect: [{ id: keeperBekasi.id }] } },
  });
  await prisma.pool.update({
    where: { id: poolSurabaya.id },
    data: { keepers: { connect: [{ id: keeperSurabaya.id }] } },
  });
  ...
  // --- Data skenario: satu order untuk tiap tahap alur, supaya semua role
  //     punya sesuatu untuk dikerjakan begitu login pertama kali ---

  // 1. PENDING — menunggu konfirmasi Marketing
  ...
  // 2. CONFIRMED — menunggu Operation menugaskan driver
  await prisma.order.upsert({
  ...
  // 3. ASSIGNED — menunggu Driver 1 melakukan pickup
  await prisma.order.upsert({
  ...
  // 4. REPORTED_AT_POOL — menunggu Penjaga Pool Bekasi memverifikasi
  await prisma.order.upsert({
  ...
  // Laporan checkpoint dari driver yang menunggu verifikasi penjaga pool
  const tripReported = await prisma.trip.findUnique({
    where: { tripNumber: 'TRP-REPORTED-004-A' },
  });
  if (tripReported) {
    const existingCheckpoint = await prisma.tripCheckpoint.findFirst({
      where: { tripId: tripReported.id },
    });
  ...
  console.log(`Login internal (password: ${DEFAULT_PASSWORD}):`);
  console.log('  admin@pss.co.id, marketing@pss.co.id, operation@pss.co.id');
  console.log('  finance@pss.co.id, management@pss.co.id');
  console.log('  driver1@pss.co.id, driver2@pss.co.id');
  console.log('  keeper.bekasi@pss.co.id, keeper.surabaya@pss.co.id');
  console.log('Login pelanggan (via OTP): customer@abc.co.id');
}
